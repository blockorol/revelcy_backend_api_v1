...
pub async fn confirm_login(
    ...
    let (user, is_new_user) =
        match auth_service::get_or_create_user_info(pool, &wallet.address).await {
...
async fn wallet_info(pubkey: web::Path<String>) -> HttpResponse {
    let rpc_url = match config::get_solana_rpc() {
        Ok(url) => url,
        Err(_) => {
            return HttpResponse::InternalServerError().json(serde_json::json!({
                "error": format!("{} environment variable not set", config::SOLANA_RPC_ENV)
            }))
        }
    };
