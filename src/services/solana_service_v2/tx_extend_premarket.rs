...
#[derive(Debug, Clone)]
pub struct ParsedExtendPremarketTx {
...
pub fn parse_extend_premarket_tx_from_base64(
    tx_b64: &str,
    network: SolanaNetwork,
) -> Result<ParsedExtendPremarketTx> {
    ...
    let new_deadline = args
        .end_timestamp
        .ok_or_else(|| anyhow!("extend tx: end_timestamp must be Some(..)"))?;

    let updated_flag = args
        .end_timestamp_updated
        .ok_or_else(|| anyhow!("extend tx: end_timestamp_updated must be Some(true)"))?;

    if !updated_flag {
        return Err(anyhow!("extend tx: end_timestamp_updated must be true"));
    }
