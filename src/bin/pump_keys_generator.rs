...
use revelcy_backend_api::config;
...
async fn main() -> Result<()> {
    ...
    if count < MIN_KEYS {
        ...
        info!(
            "Need {} more with {}; starting grind loop…",
            needed, target_suffix
        );
        for i in 0..needed {
...
async fn current_count(pool: &PgPool) -> Result<i64> {
    let rec: (i64,) = sqlx::query_as(
        "SELECT COUNT(*) FROM signing_keys WHERE premarket_pubkey = 'does_not_exist'",
    )
    .fetch_one(pool)
    .await?;
    Ok(rec.0)
...
async fn grind_store_one(pool: &PgPool, target_suffix: &str) -> Result<String> {
    ...
    let num_threads = if GRIND_THREADS > 0 {
        Some(GRIND_THREADS)
    } else {
        None
    };
    info!(
        "Starting grind for suffix '{}' (threads: {})",
        target_suffix,
    ...
    insert_mint_signing_key(pool, premarket_account_str, &pubkey_b58, &priv_key_str).await?;
